...
{"dispatch":"...","subStrategy":"...","followWithQuestion":false,"directive":"...","mood":"..."}`;
    ...
    const call = llm.invoke([new SystemMessage(sys), new HumanMessage("Decide now. JSON only.")])
        ...
        .catch(() => null);
    // clearTimeout on settle so the 1800ms timer doesn't linger per turn.
    let timer;
    const timeout = new Promise(res => { timer = setTimeout(() => res(null), 1800); });
    const d = await Promise.race([call, timeout]).finally(() => clearTimeout(timer));
