...
/**
 * Purge all cached RAG results for a given sessionId.
 * Called automatically when a new vectorStore is registered so that
 * stale content from a previous document never leaks into a new interview.
 */
function clearRagCacheForSession(sessionId) {
    ...
    if (cleared > 0) {
        interviewLog.info({ sessionId, cleared }, 'Purged stale RAG cache entries');
    }
}

export function registerVectorStore(sessionId, vectorStore, documentName = "") {
    // Always clear cached RAG results when (re-)registering a vector store
    // so we never serve stale content from a previously uploaded document.
    clearRagCacheForSession(sessionId);
    // Attach the document name so curriculum planner can reference it
    if (documentName) vectorStore._documentName = documentName;
    vectorStoreRegistry.set(sessionId, vectorStore);
    interviewLog.info({ sessionId, documentName: documentName || 'unknown' }, 'VectorStore registered (RAG cache purged)');
}

/**
 * Fully clean up all in-memory state for a session.
 * Call this when a session is destroyed or a new document is uploaded.
 */
export function clearSessionInterviewState(sessionId) {
    clearRagCacheForSession(sessionId);
    vectorStoreRegistry.delete(sessionId);
    interviewLog.info({ sessionId }, 'Session interview state fully cleared');
}
...
async function getContext(sessionId, query, k = 3) {
    ...
    if (ragCache.has(key)) {
        interviewLog.info({ sessionId, cacheKey: key }, 'RAG cache HIT — returning cached context');
        return ragCache.get(key);
    }
    const vs = getVectorStore(sessionId);
    // Diagnostic: show which Qdrant collection we're querying
    const collName = vs?.client?.collectionName || vs?.args?.collectionName || vs?.collectionName || 'unknown';
    interviewLog.info({ sessionId, qdrantCollection: collName, query: query.substring(0, 60) }, 'RAG querying Qdrant');
...
export const interviewStateChannels = {
    ...
    // Hints given for the current question (resets on new question)
    hintsGiven:          { value: (x, y) => y ?? x, default: () => 0 },
    // Cumulative off-topic count for escalating redirects
    offTopicCount:       { value: (x, y) => y ?? x, default: () => 0 },
};
...
const INTENTS = {
    ...
    // Conversational intelligence intents
    BREAK:      "break_request",     // "can we take a break?" → pause, keep same question
    HINT:       "hint_request",      // "give me a hint" → RAG-powered hint, no score advance
    GRATITUDE:  "gratitude",         // "thank you" → brief acknowledgment, continue
    GREETING:   "greeting",          // "hello" → brief hello, redirect to interview
};
...
async function generateBackchannel(score, acousticParts) {
    // Direct Gemini call — NO fallback chain (avoids 12s OpenRouter fallback)
    // Hard 800ms timeout — if slow/fails, skip backchannel gracefully
    const vibe = score >= 7
        ? "high score — engaged/impressed"
        : score >= 4
            ? "mid score — neutral and warm"
            : "low score — encouraging";
    const acoustic = acousticParts.length > 0 ? acousticParts.join("; ") : "";
    ...
    const callPromise = gemini.invoke([
        new SystemMessage(
            `You are a live interviewer. Generate ONE short spoken acknowledgment (2-4 words) spoken aloud right after the candidate answers.

Tone: ${vibe}.${acoustic ? ` Candidate: ${acoustic}.` : ""}

Examples: "Hmm nice one" / "Oh I see" / "Okay fair enough" / "That makes sense" / "Alright yeah" / "No worries"

RULES: 2-4 words only. No punctuation. No quotes. Sound natural when spoken.`
        ),
        new HumanMessage("Generate now.")
    ]).then(r => r.content.trim().replace(/["""'.]/g, "").trim())
      .catch(() => "");

    // clearTimeout on settle: without it the 800ms timer stays armed and keeps
    // the event loop referenced after the LLM already won the race.
    let timer;
    const timeoutPromise = new Promise(resolve => { timer = setTimeout(() => resolve(""), 800); });
    return Promise.race([callPromise, timeoutPromise]).finally(() => clearTimeout(timer));
}
...
async function planCurriculumNode(state) {
    ...
    interviewLog.info({ sessionId: state.sessionId, contextPreview: context?.substring(0, 200) }, 'Curriculum RAG context retrieved');

    const userProfileBlock = state.userProfileContext
        ? `\n\nOPTIONAL — CANDIDATE HISTORY (from previous interviews on OTHER documents):\n${state.userProfileContext}\nNOTE: This history is for adjusting difficulty/angles ONLY. Do NOT pull topics from this history — ALL topics must come from the CURRENT document above.`
        : "";

    // Get document name from the session for grounding
    const vs = vectorStoreRegistry.get(state.sessionId);
    const docName = vs?._documentName || "";

    const llm = makeLLM(0.3, 350);
    const response = await llm.invoke([
        new SystemMessage(`You are planning an interview curriculum based STRICTLY on the document excerpts below.

=== CURRENT DOCUMENT${docName ? `: "${docName}"` : ""} ===
${context}
=== END DOCUMENT ===
...
[
  { "topic": "<specific topic FROM THE DOCUMENT ABOVE>", "difficulty": "easy",   "angle": "<what aspect to probe>" },
  { "topic": "<specific topic FROM THE DOCUMENT ABOVE>", "difficulty": "medium", "angle": "<what aspect to probe>" },
  ...
]

STRICT Rules:
- CRITICAL: Every single topic MUST be extracted from the document excerpts shown above. If the document is about "Voice AI Agency Pricing", your topics must be about pricing, costs, tiers, etc. — NOT about DNS, networking, or any other subject.
- NEVER use your general knowledge, training data, or the candidate's history below to invent topics
- If the document context is empty or says "[NO DOCUMENT CONTENT FOUND]", generate topics using ONLY generic labels like "document overview", "key details", "main concepts"
- Each topic must be distinct — no repetition
- Difficulty: first 1-2 easy, middle medium, last 1-2 hard
- angle = the specific aspect to test (e.g. "definition", "how it works", "tradeoffs", "real-world use")
${userProfileBlock}`),
        new HumanMessage("Generate the curriculum plan now. Remember: topics MUST come from the document above, not from candidate history.")
    ]);
...
async function routerNode(state) {
    ...
    // ── FAST-PATH: META questions about interview progress (no LLM needed) ─
    const metaRegex = /\b(how many (questions?|more)|questions? (left|remaining|more|still)|what(?:'s| is) my (score|result|grade)|how am i doing|am i doing (well|okay|good|ok)|how (long|much longer)|when (will|does) (this|it) end|is this the (last|final))\b/i;
    if (metaRegex.test(state.userAnswer)) {
        interviewLog.info({ answer: state.userAnswer.substring(0, 60) }, 'Fast-path META detected');
        return { intent: INTENTS.META };
    }
    ...
    if (wordCount <= 2) {
        ...
        if (/^(thanks|thank you|thank|cheers)$/.test(lower)) {
            interviewLog.info({ wordCount, lower }, 'Fast-path GRATITUDE detected');
            return { intent: INTENTS.GRATITUDE };
        }
        if (/^(hello|hi|hey|howdy)$/.test(lower)) {
            interviewLog.info({ wordCount, lower }, 'Fast-path GREETING detected');
            return { intent: INTENTS.GREETING };
        }
        // Pure filler = thinking out loud ("um", "uh", "hmm")
        ...
        // Short content words (e.g. "coding", "arrays") → let LLM decide, not NERVOUS
        // Only flag NERVOUS for multi-word hesitation phrases
        if (wordCount === 1) {
            interviewLog.info({ wordCount, lower }, 'Short content word — routing to LLM');
            // Fall through to LLM classifier below
        } else {
            interviewLog.info({ wordCount }, 'Fast-path NERVOUS detected (2 words, non-stop)');
    ...
    if (wordCount <= 6) {
        ...
        // Break/pause requests: "can we take a break", "give me a minute", "hold on"
        if (/\b(take a break|give me a (minute|moment|second)|hold on|wait a (minute|moment|second)|one moment|pause|need a minute|need a break)\b/.test(lowerNorm)) {
            interviewLog.info({ wordCount, lowerNorm }, 'Fast-path BREAK detected');
            return { intent: INTENTS.BREAK };
        }
        // Hint requests: "give me a hint", "any clue", "help me"
        if (/\b(hint|clue|help me|give me a clue|give me a hint|any hints|need help|some help)\b/.test(lowerNorm)) {
            interviewLog.info({ wordCount, lowerNorm }, 'Fast-path HINT detected');
            return { intent: INTENTS.HINT };
        }
        // Gratitude (multi-word): "thank you so much", "thanks a lot"
        if (/\b(thank you|thanks a lot|thanks so much|appreciate it|appreciate that)\b/.test(lowerNorm)) {
            interviewLog.info({ wordCount, lowerNorm }, 'Fast-path GRATITUDE detected');
            return { intent: INTENTS.GRATITUDE };
        }
        // Greeting (multi-word): "good morning", "hey there"
        if (/\b(good morning|good afternoon|good evening|hey there|hello there)\b/.test(lowerNorm)) {
            interviewLog.info({ wordCount, lowerNorm }, 'Fast-path GREETING detected');
            return { intent: INTENTS.GREETING };
        }
    }
...
- unwell           → health issue ("I'm sick", "I don't feel well")
...
- break_request    → wants a brief pause ("can we take a break?", "give me a minute", "hold on a second", "I need a moment")
- hint_request     → asking for help with the answer ("can you give me a hint?", "any clues?", "I need some help", "can you help me out?")
- gratitude        → thanking the interviewer ("thank you", "thanks for explaining", "appreciate it", "that was helpful")
- greeting         → greeting the interviewer ("hello", "good morning", "hey there", "hi how are you")
...
- "take a break" / "need a minute" → break_request
- "give me a hint" / "help me" → hint_request`;
...
async function gradeAnswerNode(state) {
    ...
    const gradeQuery = state.topicTag
        ? `${state.topicTag}: ${state.currentQuestion}`
        : state.currentQuestion;
    const context = await getContext(state.sessionId, gradeQuery);
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
{
  ...
  "follow_up_angle": "<specific probe direction or empty string>"
    ...
    // Grade with Groq first (~600ms)
    const llm = makeLLM(0.1, 340);
    const response = await llm.invoke([
        new SystemMessage(systemPrompt),
    ...
    // Gemini backchannel with real score — fast (~100ms), Gemini flash-lite 20 tokens
    const backchannelText = await generateBackchannel(data.score || 5, acousticParts);
    ...
    // Prepend Gemini-generated backchannel — single natural spoken phrase
    const backchannel = (backchannelText || "").replace(/["""]/g, "").trim();
    const naturalFeedback = backchannel && cleanFeedback
        ? `${backchannel}, ${cleanFeedback}`
        : cleanFeedback;

    // Classify quality (for scoring/UI only — NO tag prefix on feedback)
    // Backchannel replaces cached phrase tags: no more disjointed "Good effort. [pause] Okay fair enough..."
    ...
    // feedback = clean natural speech: "Okay fair enough, you covered the basics..."
    const taggedFeedback = naturalFeedback;

    // Cap score if candidate used hints on this question
    let finalScore = data.score || 0;
    if ((state.hintsGiven || 0) > 0 && finalScore > 7) {
        interviewLog.info({ originalScore: finalScore, hintsGiven: state.hintsGiven }, 'Score capped due to hint usage');
        finalScore = 7;
    }
...
async function handleEdgeCaseNode(state) {
    // ── BREAK REQUEST: acknowledge pause, keep same question ────────
    if (state.intent === INTENTS.BREAK) {
        interviewLog.info({ sessionId: state.sessionId }, 'Break requested — pausing');
        const answer = "Of course, take your time. Whenever you're ready, just start speaking and we'll continue right where we left off.";
        return {
    ...
    // ── HINT REQUEST: provide RAG-powered hint without giving the answer ──
    if (state.intent === INTENTS.HINT) {
        const currentHints = state.hintsGiven || 0;
        if (currentHints >= 2) {
            interviewLog.info({ sessionId: state.sessionId, hintsGiven: currentHints }, 'Max hints reached');
            const answer = "I've already given you a couple of hints on this one. Give it your best shot, or say 'skip' if you'd like to move on.";
            return {
                chatHistory: [...state.chatHistory, new HumanMessage(state.userAnswer), new AIMessage(answer)],
                interviewStopped: false,
                evaluation: { score: 0, accuracy: 0, clarity: 0, depth: 0, feedback: answer, nextDifficulty: "same" }
        ...
        interviewLog.info({ sessionId: state.sessionId, hintsGiven: currentHints }, 'Generating hint from RAG context');
        const context = await getContext(state.sessionId, state.currentQuestion, 3);
        const llm = makeLLM(0.5, 100);
        const response = await llm.invoke([
            new SystemMessage(`You are a helpful interviewer giving a hint. The candidate is stuck on a question.
Do NOT give the answer directly. Instead, nudge them toward the right direction with a conceptual clue.
Be warm and encouraging. Keep it to 1-2 sentences.

QUESTION: "${state.currentQuestion}"
REFERENCE MATERIAL: ${context?.substring(0, 500) || 'No context available'}
HINT NUMBER: ${currentHints + 1} of 2
${currentHints === 0 ? 'Give a broad conceptual nudge.' : 'Give a more specific hint since the first one was not enough.'}`),
            new HumanMessage("Give the hint now.")
        ]);
        const hint = response.content.trim();
        interviewLog.info({ sessionId: state.sessionId, hint: hint.substring(0, 100) }, 'Hint generated');

        return {
            hintsGiven: currentHints + 1,
            chatHistory: [...state.chatHistory, new HumanMessage(state.userAnswer), new AIMessage(hint)],
            interviewStopped: false,
            evaluation: { score: 0, accuracy: 0, clarity: 0, depth: 0, feedback: hint, nextDifficulty: "same" }
        };
    }

    // ── GRATITUDE: brief warm acknowledgment, continue interview ────
    if (state.intent === INTENTS.GRATITUDE) {
        interviewLog.info({ sessionId: state.sessionId }, 'Gratitude received');
        const answer = "You're welcome! Let's keep going.";
        return {
            chatHistory: [...state.chatHistory, new HumanMessage(state.userAnswer), new AIMessage(answer)],
            interviewStopped: false,
            evaluation: { score: 0, accuracy: 0, clarity: 0, depth: 0, feedback: answer, nextDifficulty: "same" }
    ...
    // ── GREETING: brief hello, redirect to interview ────────────────
    if (state.intent === INTENTS.GREETING) {
        interviewLog.info({ sessionId: state.sessionId }, 'Greeting received');
        const answer = "Hey there! Nice to meet you. Let's continue with the interview.";
        return {
            chatHistory: [...state.chatHistory, new HumanMessage(state.userAnswer), new AIMessage(answer)],
            interviewStopped: false,
            evaluation: { score: 0, accuracy: 0, clarity: 0, depth: 0, feedback: answer, nextDifficulty: "same" }
    ...
    if (state.intent === INTENTS.META) {
        ...
        let answer;
        if (lower.match(/\b(remain|left|more|how many|questions|total)\b/)) {
            answer = remaining === 0
                ? "This is actually the last question — you're almost done!"
                : `You have ${remaining} question${remaining !== 1 ? 's' : ''} remaining after this one, out of ${state.maxQuestions} total.`;
        ...
        } else if (lower.match(/\b(hint|help|clue)\b/)) {
            // Meta question about hints — tell them they can ask
            answer = "You can ask me for a hint anytime! Just say 'give me a hint' and I'll try to nudge you in the right direction.";
        } else {
            answer = avgScore
    ...
    // ── IRRELEVANT: smarter off-topic handling with escalation ──────
    if (state.intent === INTENTS.IRRELEVANT) {
        const newOffTopicCount = (state.offTopicCount || 0) + 1;
        interviewLog.info({ sessionId: state.sessionId, offTopicCount: newOffTopicCount }, 'Off-topic comment — generating redirect');

        const lower = state.userAnswer.toLowerCase();
        let redirect = "";

        // Check for personal/AI identity questions — respond with charm, no LLM call
        if (/\b(your name|who are you|are you (a |an )?(robot|ai|human|real|machine|computer)|whats your name)\b/.test(lower)) {
            redirect = "I'm your AI interviewer! I don't have a name, but I've been told I have excellent questioning skills. Now, let's get back to the interview.";
        } else if (newOffTopicCount >= 3) {
            // Escalated redirect after repeated off-topic
            redirect = "I appreciate the conversation, but we should really focus on the interview now. Let's get back to the question I asked.";
        } else {
            // LLM-generated warm redirect
            const llm = makeLLM(0.7, 80);
            const prompt = `The candidate said something off-topic during an interview: "${state.userAnswer}". Write ONE brief, friendly sentence that acknowledges their comment and redirects back. Be warm, not dismissive.`;
            const res = await llm.invoke([
                new SystemMessage("Write natural spoken responses. No markdown, no lists. Maximum one sentence."),
...
async function prefetchContextNode(state) {
    try {
        ...
        const query = planItem
            ? `${planItem.topic}: ${planItem.angle}. definitions examples tradeoffs how it works`
            : "core concepts definitions key ideas examples tradeoffs";
...
- Example good follow-up: "You mentioned X — can you walk me through how that actually works under the hood?"`;
        ...
        if (socket) {
            const llm = makeLLM(0.7, 120, true);
            const stream = await llm.stream([
                new SystemMessage(systemPrompt),
    ...
    const isRepeat = state.intent === INTENTS.NERVOUS || state.intent === INTENTS.IRRELEVANT || state.intent === INTENTS.GREETING;
...
- Sound like a real human interviewer, not a script. Occasionally use natural spoken openers like "So...", "Okay, so...", "Right, and...", "Let me ask you this —" — but only once every 2-3 questions, not every time`;
    ...
    return {
        ...
        hintsGiven: 0,          // Reset hints for the new question
...
function afterEdgeCaseRoute(state) {
    ...
    // These intents are handled entirely inside handleEdgeCaseNode —
    ...
    // Conversational intents — speak response, stay on same question
    if (state.intent === INTENTS.BREAK || state.intent === INTENTS.HINT || state.intent === INTENTS.GRATITUDE || state.intent === INTENTS.GREETING) return END;
    return "generateQuestion";
}
...
// ═══════════════════════════════════════════════════════════════════
// SHARED EXPORTS — reused by the Deep Interview Agent (deepInterviewAgent.js)
// Additive only. Does not change any existing behaviour.
// ═══════════════════════════════════════════════════════════════════
export {
    // helpers
    getContext,
    ...
    buildCompactHistory,
    INTENTS,
    // node functions reused by the deep agent's orchestration
    planCurriculumNode,
    gradeAnswerNode,
    backchannelNode,
    prefetchContextNode,
