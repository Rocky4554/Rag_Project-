...
async function planCurriculumNode(state) {
    console.log("[Interview] Planning curriculum from document…");
...
- angle = the specific aspect to test (e.g. "definition", "how it works", "tradeoffs", "real-world use")`),
    ...
    try {
        ...
        console.log(`[PlanCurriculum] Parse failed: ${err.message}. Using default plan.`);
    ...
    console.log(`[PlanCurriculum] Ready: ${plan.map(p => `${p.topic}(${p.difficulty})`).join(" → ")}`);
...
async function processAnswerNode(state) {
    ...
    if (wordCount < 5) {
        console.log(`[ProcessAnswer] Short response (${wordCount} words) → routing as nervous`);
        return {
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
{
  ...
  "keyConceptMissed": "<specific missed concept or empty string>"
    ...
    try {
        ...
        console.log(`[ProcessAnswer] JSON parse failed: ${err.message}. Defaulting.`);
        data = {
    ...
    console.log(`[ProcessAnswer] Intent: ${validatedIntent} | Quality: ${data.answerQuality} | Score: ${data.score} | FollowUp: ${shouldFollowUp} | Streak: ${newStruggleStreak}`);
    console.log(`[ProcessAnswer] Topic: "${tag}" | Missed: "${data.keyConceptMissed || "—"}"`);
...
async function handleEdgeCaseNode(state) {
    ...
    if (cachedKey) {
        ...
        console.log(`[EdgeCase] Intent "${state.intent}" → cached TTS: ${cachedKey}.mp3 ${shouldEndNow ? "[FINALIZE]" : ""}`);
...
async function prefetchContextNode(state) {
    try {
        ...
        console.log(`[Prefetch] Fetching context for: "${query}"`);
...
async function generateQuestionNode(state) {
    ...
    if (state.followUpFlag) {
        console.log(`[Interview] Generating follow-up for Q#${state.questionsAsked} (topic: "${state.topicTag}")`);
...
- Example good follow-up: "You mentioned X — can you walk me through how that actually works under the hood?"`;
        ...
        console.log(`[Interview] Follow-up: "${questionText.substring(0, 80)}…"`);
    ...
    if (isRepeat && (state.repeatCount || 0) >= 2) {
        console.log(`[Interview] Max repeats (${state.repeatCount}) reached — force-advancing.`);
    ...
    console.log(`[Interview] Q#${nextQ}/${state.maxQuestions} | Topic: "${planItem.topic}" | Difficulty: ${planItem.difficulty} ${effectiveRepeat ? "[REPEAT]" : ""} ${supportiveTone ? "[SUPPORTIVE]" : ""}`);
