    ...
    async embedDocuments(texts) {
        ...
        for (let i = 0; i < texts.length; i++) {
            ...
            for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
                try {
                    // Use embedQuery to avoid the base class swallowing errors and returning [[]]
                    const vec = await this.base.embedQuery(texts[i]);
                    if (vec && vec.length > 0) {
                        embedding = vec;
                        break;
                    } else {
                    ...
                    console.warn(`  Embedding attempt ${attempt} failed for chunk ${i + 1}: ${err.message}`);
                    // If it's a 400 Bad Request (like API Key expired) or 403 Forbidden, don't retry.
...
export async function storeDocuments(docs, collectionName = "my_collection") {
    ...
    // We use the separate parameters to avoid the 'path' deprecation warning in Chroma client.
    const vectorStore = await Chroma.fromDocuments(sanitizedDocs, retryEmbedder, {
        collectionName: collectionName,
        url: "http://localhost:8000", // LangChain default
        // Passing host/port/ssl to avoid deprecation warnings if the underlying client supports it
        chromaClientArgs: {
            host: "localhost",
            port: 8000,
            ssl: false
    ...
    // Note: The "No embedding function" warning from ChromaDB is often non-fatal in LangChain 
    // because LangChain manages the embeddings itself via RetryEmbedder. 
    // However, the above chromaClientArgs helps with the connection warning.
