...
function ensureCacheLoaded() {
    ...
    try {
        ...
        console.warn(`[EmbeddingCache] Failed to load cache: ${err.message}. Using in-memory only.`);
        embeddingCache = new Map();
    }
...
function persistCache() {
    ...
    try {
        ...
        fs.writeFileSync(EMBEDDING_CACHE_PATH, JSON.stringify(obj));
    } catch (err) {
        console.warn(`[EmbeddingCache] Failed to persist cache: ${err.message}`);
    }
}
...
class BatchCachingEmbedder {
    ...
    async embedDocuments(texts) {
        ...
        if (toEmbed.length > 0) {
            ...
            console.log(
                `[Embeddings] total=${texts.length} cacheHits=${cacheHits} toEmbed=${toEmbed.length} batches=${batches.length} batchSize=${BATCH_SIZE} concurrency=${BATCH_CONCURRENCY}`
            );
            ...
            await runWithConcurrency(
                ...
                async (batch, batchIdx) => {
                    ...
                    try {
                        ...
                        console.warn(`  Batch #${batchNumber} fallback: isolating items due to "${batchErr.message}"`);
                        for (let i = 0; i < batch.length; i++) {
                            const item = batch[i];
                            ...
                            try {
                                ...
                                console.warn(`    Skipping unembeddable item #${i + 1} in batch #${batchNumber}: ${itemErr.message}`);
...
export async function storeDocuments(docs, collectionName = "my_collection") {
    ...
    console.log(`Storing ${docs.length} document chunks in Qdrant collection "${qdrantCollection}" (batched + cache + retry)...`);
    ...
    console.log(`All ${sanitizedDocs.length} chunks stored successfully in Qdrant.`);
    return vectorStore;
}
