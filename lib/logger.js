...
if (OTLP_ENABLED) {
    ...
    // Flush only — do NOT exit here. server.js owns shutdown (stopping live
    // agents so their LiveKit participants don't linger); calling process.exit
    // from this listener would run first and cut that cleanup off entirely.
    process.on('SIGTERM', flushOtlpLogs);
    process.on('SIGINT', flushOtlpLogs);
}
