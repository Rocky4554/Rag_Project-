...
export async function routeQuery(session, query, chatHistory = [], userName = null) {
    ...
    try {
        ...
        const answer = await queryRAG(session.vectorStore, session.bm25Index, query, chatHistory);
    ...
    const answer = await queryRAG(session.vectorStore, session.bm25Index, effectiveQuery, chatHistory);
    return { route: "RAG", answer };
}
...
export async function* routeQueryStream(session, query, chatHistory = [], userName = null) {
    ...
    try {
        ...
        yield* queryRAGStream(session.vectorStore, session.bm25Index, query, chatHistory);
        return;
    }
    ...
    const effectiveQuery = classification.refinedQuery || query;
    yield* queryRAGStream(session.vectorStore, session.bm25Index, effectiveQuery, chatHistory);
}
