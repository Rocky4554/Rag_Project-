...
export async function queryRAG(vectorStore, bm25Index, query, chatHistory = []) {
...
Keep answers concise and conversational. Do not refuse to answer — always be helpful.`
    ...
    // 4. Build combine chain.
    const combineDocsChain = await createStuffDocumentsChain({
        llm: llm,
        prompt: qaPrompt,
    });

    // 5. Hybrid retrieval: BM25 + semantic → RRF merge → rerank → top 10
    const retrievalStart = performance.now();
    const retrievedDocs = await hybridRetrieve(vectorStore, bm25Index, contextualQuery, query, { topK: 10 });
 ...
 */
export async function* queryRAGStream(vectorStore, bm25Index, query, chatHistory = []) {
    const ragStart = performance.now();
...
Keep answers concise and conversational. Do not refuse to answer — always be helpful.`
    ...
    // Hybrid retrieval: BM25 + semantic → RRF merge → rerank → top 10
    const retrievalStart = performance.now();
    const retrievedDocs = await hybridRetrieve(vectorStore, bm25Index, contextualQuery, query, { topK: 10 });
    const retrievalMs = Math.round(performance.now() - retrievalStart);
