...
export async function deleteDocument(documentId, userId) {
    // Delete related records first (cascade manually). These three target
    // different tables and share no dependency, so they run concurrently —
    // serially they were three full Supabase round trips back to back.
    await Promise.all([
        supabaseAdmin.from('chat_messages').delete().eq('document_id', documentId).eq('user_id', userId),
        supabaseAdmin.from('interview_results').delete().eq('document_id', documentId).eq('user_id', userId),
        supabaseAdmin.from('quiz_results').delete().eq('document_id', documentId).eq('user_id', userId),
    ]);
