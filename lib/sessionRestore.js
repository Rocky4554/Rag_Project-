...
import { loadBM25Index } from "./pipeline/bm25Index.js";
...
export async function restoreSession(sessionId) {
    try {
        ...
        // Text/PDF sessions: reconnect to Qdrant via LangChain + reload BM25 index from disk
        const [vectorStore, bm25Index] = await Promise.all([
            QdrantVectorStore.fromExistingCollection(embedder, {
                url: process.env.QDRANT_URL,
                apiKey: process.env.QDRANT_API_KEY,
                collectionName,
            }),
            loadBM25Index(sessionId),
        ]);

        serverLog.info(
            { sessionId, collectionName, chatMessages: chatHistory.length, hasBM25: !!bm25Index },
            'Session restored from DB + Qdrant'
        );
